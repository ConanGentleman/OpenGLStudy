...
"void main()\n"
"{\n"
"   gl_Position = transform * vec4(aPos.x,aPos.y,aPos.z, 1.0);//gl_Position是OpenGL的标准化坐标所以是vec4\n"
    ...
    */
    while (!glfwWindowShouldClose(window))
    {
        ...
        if (transformLoc != -1) {
            ...
            glUniformMatrix4fv(transformLoc, 1, GL_FALSE, glm::value_ptr(trans));//位置，矩阵数量，是否转置，
        }
        trans = glm::mat4(1.0f);
        ...
        if (transformLoc != -1) {
            scaleOP();
            glUniformMatrix4fv(transformLoc, 1, GL_FALSE,&trans[0][0]);//这次取矩阵值数组的第一个元素作为它的内存指针值
        }
        //glBindVertexArray(VAO);
        glDrawElements(GL_TRIANGLES, 6, GL_UNSIGNED_INT, 0);
