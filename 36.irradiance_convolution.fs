...
{		
    ...
    float sampleDelta = 0.025;// 固定的sampleDelta 增量值遍历半球
    float nrSamples = 0.0;
    //遍历半球
    for(float phi = 0.0; phi < 2.0 * PI; phi += sampleDelta)//航向角 ϕ
    {
        for(float theta = 0.0; theta < 0.5 * PI; theta += sampleDelta)//从半球顶点出发的倾斜角 θ
        {
            // 球面到笛卡儿(切线空间)
