...
"vec3 lightDir = normalize(lightPos - FragPos); //光照方向，并归一化\n"
"float diff = max(dot(norm, lightDir), 0.0);//求光照与法线的夹角cos \n"
"vec3 diffuse = diff * lightColor;//得到漫反射项的光照 \n"
"// specular\n"
...
"float spec = pow(max(dot(viewDir, reflectDir), 0.0), 32);//视角与反射方向的cos \n"
"vec3 specular = specularStrength * spec * lightColor;//镜面反射分量 \n"
"vec3 result = (ambient + diffuse + specular) * objectColor;//将环境光照与漫反射项体现在物体上 \n"
...
int main()
{
    ...
    */
    while (!glfwWindowShouldClose(window))
    {
        ...
        transformLoc = glGetUniformLocation(shaderProgram, "lightPos");
