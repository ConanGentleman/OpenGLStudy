...
int main()
{
    ...
    //0.1. 顶点数组对象（解决一个一个绘制的问题
    unsigned int VAO;
    glGenVertexArrays(1, &VAO);
    //0.2. 元素缓冲区对象(解决绘制矩形时，通过两个三角形绘制需要绘制6个顶点而不是4个，可以处理重合顶点的情况）
    unsigned int EBO;
    glGenBuffers(1, &EBO);//OpenGL允许我们同时绑定多个缓冲，只要它们是不同的缓冲类型
    // 1. 绑定VAO
    glBindVertexArray(VAO);
    // 1.0 绑定EVO
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, EBO);
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(indices), indices, GL_STATIC_DRAW);//GL_ELEMENT_ARRAY_BUFFER当作缓冲目标
    ...
    glDeleteShader(vertexShader);//链接后的着色器保存在了shaderProgram中，
    glDeleteShader(fragmentShader);//原来的着色器就可以删除了
    ...
    */
    while (!glfwWindowShouldClose(window))
    {
        ...
        //glDrawArrays(GL_TRIANGLES, 0, 6);//使用VAO绘制三角形,参数为：图元类型，起始顶点索引和画多少个
