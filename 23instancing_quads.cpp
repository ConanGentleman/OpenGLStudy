...
int main()
{
    ...
    Shader shader("23.instancing.vs", "23.instancing.fs");

    // 生成100个四边形位置/平移向量的列表(每个四边形的偏移位置）
    // ---------------------------------------------------------
    ...
    glBindBuffer(GL_ARRAY_BUFFER, 0);
    glVertexAttribDivisor(2, 1); // 告诉OpenGL这是一个实例化的顶点属性。表示 对于需要的顶点属性2，每渲染1个新实例的时候更新顶点属性
